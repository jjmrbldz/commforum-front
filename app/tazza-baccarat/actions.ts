...
import { getCookieData, requireUserSession } from "@/lib/session";
...
export async function openGame(): ServerActionResponse {
  try {
    const user = await requireUserSession();
    if (!user) {
      throw new Error("User not authenticated");
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACC_API_URL}/auth/open-game`,
      {
        method: "POST",
        headers: {
    ...
    if (data?.statusCode !== 200) {
      return { 
        ok: false, 
        message: data?.message || "Open game failed." 
      }
    }
