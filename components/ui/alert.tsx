  ...
  "relative w-full border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        info:
          "text-blue-600 bg-current/10 [&>svg]:text-current *:data-[slot=alert-description]:text-blue-600/90",
        warn:
          "text-yellow-600 bg-current/10 [&>svg]:text-current *:data-[slot=alert-description]:text-yellow-600/90",
      },
    },
    defaultVariants: {
      variant: "default",
